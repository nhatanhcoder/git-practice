...
async function createActor(role: 'teacher' | 'admin' | 'student', tag: string): Promise<Actor> {
  ...
  userIds.push(row.id);
  const login = await request('POST', '/auth/login', undefined, { email: row.email, password });
...
after(async () => {
  if (prisma) {
    ...
    await prisma.learningPath.deleteMany({ where: { ownerId: { in: userIds } } });
...
describe('Teacher-authored Learning Catalog invariants', () => {
  it('INV-LMOD-01 and RBAC branches: role-prefixed endpoints fail closed', async () => {
    ...
    assert.equal((await request('POST', '/teacher/learning-paths', adminA.token, { title: 'No author' })).status, 403);
  });
  ...
  it('INV-LCAT-01/02/12: ownership is enforced and private fields stay off wire', async () => {
    ...
    assert.equal((await request('PATCH', `/teacher/learning-paths/${pathA.id}`, teacherB.token, { title: 'Đổi sai chủ' })).status, 403);
  ...
  it('INV-LMOD-02/05/08: concurrent approve has one winner, one notification and complete audit', async () => {
    ...
    assert.equal(
      await prisma.notification.count({
        where: { type: 'learning_path_approved', referenceId: pathA.id, userId: teacherA.id },
      }),
      1,
    );
    const stored = await prisma.learningPath.findUniqueOrThrow({ where: { id: pathA.id } });
  ...
  it('INV-LCAT-06/07/08/14: publish is approved-only, immutable after first publish and unpublish preserves progress', async () => {
    ...
    assert.equal((await request('POST', `/teacher/learning-units/${unitA.id}/unpublish`, teacherA.token, {})).status, 200);
    assert.equal(await mongo.collection('user_learning_progress').countDocuments({ userId: student.id, unitSlug: unitA.slug }), 1);
    assert.deepEqual((await mongo.collection('learning_units').findOne({ slug: unitA.slug }))?.words, originalWords);
    assert.equal((await request('POST', `/teacher/learning-units/${unitA.id}/publish`, teacherA.token, {})).status, 200);
    assert.deepEqual((await mongo.collection('learning_units').findOne({ slug: unitA.slug }))?.words, originalWords);
  });

  it('INV-LCAT-09: reorder accepts only the complete 1..N permutation', async () => {
    const second = await request('POST', `/teacher/learning-paths/${pathA.id}/units`, teacherA.token, authored('Bài thứ hai'));
    ...
    assert.deepEqual(valid.data.units.map((unit: any) => unit.order), [1, 2]);
  });
  ...
  it('INV-LCAT-10: reference keeps no copied words and rejects a suspended source path', async () => {
    ...
    unitB = (await request('POST', `/teacher/learning-paths/${pathB.id}/units`, teacherB.token, authored('Nguồn công sở'))).data;
  ...
  it('INV-LMOD-03/12: reject reason is exact, visible to owner and only owner can resubmit', async () => {
    ...
    const unit = await request('POST', `/teacher/learning-paths/${created.data.id}/units`, teacherA.token, authored('Bài chờ từ chối'));
    ...
    assert.equal((await request('POST', `/teacher/learning-paths/${created.data.id}/submit`, teacherA.token, {})).status, 200);
    assert.equal((await request('PATCH', `/admin/learning-paths/${created.data.id}/approve`, adminA.token, {})).status, 200);
  ...
  it('INV-LCAT-11: delete is blocked by approval, publication history or learner progress', async () => {
    ...
    const progressPath = (await request('POST', '/teacher/learning-paths', teacherA.token, { title: 'Draft có progress' })).data;
    const progressUnit = (await request('POST', `/teacher/learning-paths/${progressPath.id}/units`, teacherA.token, authored('Draft unit'))).data;
  ...
  it('INV-LMOD-04/10/11 and student visibility: suspend hides, restore preserves published set and progress', async () => {
    const before = await request('GET', `/student/learning-path?curriculum=${pathA.curriculumKey}&level=1`, student.token);
    const publishedBefore = before.data.units.filter((unit: any) => unit.state !== 'unavailable').map((unit: any) => unit.slug);
    ...
    assert.equal((await request('GET', `/student/learning-path?curriculum=${pathA.curriculumKey}&level=1`, student.token)).data.total, 0);
    assert.equal((await request('GET', '/student/learning-path/curricula', student.token)).data.some((row: any) => row.key === pathA.curriculumKey), false);
    assert.equal(await mongo.collection('user_learning_progress').countDocuments({ userId: student.id, unitSlug: unitA.slug }), 1);
    const restored = await request('PATCH', `/admin/learning-paths/${pathA.id}/restore`, adminB.token, {});
    assert.equal(restored.status, 200);
    const afterCatalog = await request('GET', `/student/learning-path?curriculum=${pathA.curriculumKey}&level=1`, student.token);
    const publishedAfter = afterCatalog.data.units.filter((unit: any) => unit.state !== 'unavailable').map((unit: any) => unit.slug);
    assert.deepEqual(publishedAfter, publishedBefore);
    const audit = await prisma.learningPath.findUniqueOrThrow({ where: { id: pathA.id } });
  ...
  it('INV-LCAT-10 addendum: a missing reference source remains an unavailable catalog node', async () => {
    ...
    const catalog = await request('GET', `/student/learning-path?curriculum=${pathA.curriculumKey}&level=1`, student.token);
