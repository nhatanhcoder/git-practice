...
- ✅ (antigravity · 2026-09-10) **Auth Map cleanup, Trust Proxy & Throttler adoption (A1+A2+H)** — loginAttempts sweep on check/write, rotationCache expired sweep + MAX_ENTRIES eviction (10,000 FIFO), Express trust proxy, and @nestjs/throttler adoption (anti + h code). Verified via test suite auth-map-cleanup.test.ts (6/6), auth.e2e.test.ts (17/17), refresh-token-concurrency.e2e.test.ts (4/4).
