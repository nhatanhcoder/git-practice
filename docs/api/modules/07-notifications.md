...
status: implemented (updated 2026-09-20, branch `codex/learning-catalog-backend`) — the 4 mailbox endpoints, the `NOTIFICATION_NOT_FOUND` code and existing producers are live. The 4 ADR-017 Learning Catalog enum values, reference type, FE sentence/deep-link map and runtime producers are implemented; their real-DB verification remains pending BUILD-006. Still open: `deadline_reminder` scheduler, the partial-unique anti-duplicate migration (§8), and the composite/partial indexes (§11). FE builds sentences from type+payload (no message column), there is no senderId, admin fan-out targets every active admin, `read-all` → `{updated}`, `unread-count` stays separate, and `PATCH /:id/read` → 200 with the record.
